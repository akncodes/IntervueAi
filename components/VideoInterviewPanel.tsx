...
export const VideoInterviewPanel = ({ roomId }: VideoInterviewPanelProps) => {
  ...
  return (
    <div className="w-full rounded-2xl bg-dark-200 border border-dark-100 overflow-hidden shadow-2xl glassmorphism p-6 mb-8 transition-all duration-300 hover:border-emerald-500/20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 border-b border-dark-100/50 pb-4">
        <div>
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Shield className="size-5 text-emerald-500 animate-pulse" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative min-h-[340px]">
        ...
        <div className="relative rounded-xl overflow-hidden bg-dark-300/85 border border-dark-100 flex items-center justify-center aspect-video group">
              ...
              {connectionStatus === "failed" ? (
                <>
                  ...
                  <p className="text-[11px] text-gray-500 text-center max-w-[320px] bg-dark-400/50 p-2 rounded border border-dark-100/50 font-mono">
                    ...
                    <span className="text-emerald-400">npm run dev</span> inside the <code className="text-white">/backend</code> folder!
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-sm font-semibold text-emerald-400 font-mono tracking-wide animate-pulse">Awaiting AI Interviewer...</p>
          ...
          <span className="absolute top-4 left-4 bg-black/80 px-3 py-1.5 rounded-full text-[10px] font-bold tracking-widest font-mono text-emerald-400 border border-emerald-500/25 backdrop-blur-md uppercase">
            AI Recruiter (Chloe)
          </span>
        </div>
        ...
        <div className="relative rounded-xl overflow-hidden bg-dark-300/85 border border-dark-100 flex items-center justify-center aspect-video group">
          ...
          {!localStream && isVideoEnabled && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-dark-400 gap-2">
              <span className="size-6 border-2 border-t-emerald-500 border-r-transparent border-b-transparent border-l-transparent rounded-full animate-spin" />
